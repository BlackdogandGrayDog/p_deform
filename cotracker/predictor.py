...
class CoTrackerPredictor(torch.nn.Module):
    ...
    def _compute_sparse_tracks(
        ...
        if queries is not None:
            B, N, D = queries.shape
            assert D == 3
            queries_point = queries.clone()
            queries_point[:, :, 1:] *= queries_point.new_tensor(
                [
        ...
        if grid_size > 0:
            ...
            queries_grid = torch.cat(
                [torch.ones_like(grid_pts[:, :, :1]) * grid_query_frame, grid_pts],
                dim=2,
            ).repeat(B, 1, 1)

        if 'queries_point' in locals() and 'queries_grid' in locals():
            queries = torch.cat([queries_point, queries_grid], dim=1)
        elif 'queries_point' in locals():
            queries = queries_point  # Only queries_point exists
        elif 'queries_grid' in locals():
            queries = queries_grid  # Only queries_grid exists
        else:
            raise ValueError("No queries available from either source")
